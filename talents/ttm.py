    ...
    for hero_talent in hero_talents:
        ...
        for build in config["builds"]["hero"][hero_talent]:
            for line in data:
                ...
                line = line.replace("profileset.\"",f"profileset.\"{build}_")
                prefix = line.split("+=")[0]
                hero_line = f"{prefix}+=\"hero_talents={talent_string}\"\n"
                class_line = ""
                if hero_talent == "AR":
                    class_line = f"{prefix}+=\"class_talents+=/halo:1/divine_star:0\"\n"
                HERO_FILE = HERO_FILE + line + hero_line + class_line
        # replace file with new combos
        with open(f"hero_{hero_talent}_duplicated.simc", "w", encoding="utf8") as file:
            file.writelines(HERO_FILE)
            file.close()
