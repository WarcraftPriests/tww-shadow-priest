...
if __name__ == "__main__":
    ...
    with open("results/Results_Single.csv", "r", encoding="utf8") as file:
        reader = csv.reader(file)
        for row in reader:
            if talent_spec_strings.get(row[1]):
                talent_spec_strings[row[1]].dps = row[2]
        # results_dict = {rows[1]:rows[2] for rows in reader}
        file.close()

    with open("results/info.md", "w", encoding="utf8") as file:
        file.write("# Talent Info\n")
        for build in talent_builds:
            file.write(f"## {build}\n|talent|min|max|mean|\n|---|---|---|---|\n")
            print(f"Getting stats for build: {build}")
